...
export default function Page() {
  return (
    <TopicLayout slug={slug}>
      ...
      <p>
        エクステントを意識することで、
        「<a href="/clustered">クラスタ化インデックス</a>の範囲検索がなぜシーケンシャルI/Oで済むのか」が物理的に理解できます。連続するページが同一エクステント内にあれば、ディスクヘッドを大きく動かさずに読める（あるいはSSDでも先読みが効く）からです。
      </p>
      ...
      <ul>
        <li>
          <a href="/btree">B-tree</a>: 葉ノードから行IDを引き、その行が入っているページを1回読む。
        </li>
        <li>
          <a href="/clustered">クラスタ化インデックス</a>: テーブル本体のページ並びそのものがキー順になっていて、連続エクステントを一気に読める。
        </li>
        <li>
          <a href="/covering">カバリングインデックス</a>: 必要な列がインデックス側に入っているから、テーブル本体のページを読まなくていい。
        </li>
        <li>
          <a href="/cost">インデックスのコスト</a>: 1回のランダムページ読み取りは連続読み取りの数十〜数百倍遅い、というのが更新コスト議論の前提。
        </li>
      </ul>
