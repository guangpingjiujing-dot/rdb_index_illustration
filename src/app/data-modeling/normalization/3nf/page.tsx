...
export default function Page() {
  return (
    <TopicLayout section="data-modeling" slug={slug}>
      ...
      <h2>推移関数従属を排除する</h2>
      <p>
        第3正規形 (3NF) の要件は次の 2 つ:
      </p>
      <ul>
        <li><Link href="/data-modeling/normalization/2nf">第2正規形</Link> を満たしている</li>
        <li><strong>非キー属性が、候補キー以外の別の非キー属性を経由して関数従属していない</strong> (推移関数従属を持たない)</li>
      </ul>
      <p>
        <code>X → Y → Z</code> (Y は非キー属性) という経由での従属が <strong>推移関数従属</strong> (transitive functional dependency) だ。
        こういう経由があると、Y に依存する Z が「X の値ごとにいちいち一緒に書かれる」ことになり、
        <Link href="/data-modeling/normalization/why">更新時異常</Link> を生む。
      </p>

      <h2>題材: 2NF まで進めた受注テーブル</h2>
      <p>
        <Link href="/data-modeling/normalization/2nf">前ページ</Link> の 2NF 化で、
        受注データは <strong>受注 / 商品 / 受注明細</strong> の 3 テーブルに分割された。
        このうち <strong>商品</strong> と <strong>受注明細</strong> はもう分析するべき従属関係が残っていない。
        問題は <strong>受注</strong> テーブル — ここに顧客関連の列 (顧客名・顧客住所) がまだ残っている。
      </p>
      ...
      <h2>3NF 化: 推移関数従属する属性を切り出す</h2>
      <p>
        受注テーブルから顧客関連の列 (顧客名・顧客住所) を切り出し、
        <strong>顧客</strong> テーブルとして独立させる。
        受注テーブル側には <code>顧客ID</code> だけを外部キーとして残す。
      </p>
      ...
      <h2>3NF と BCNF の違い (触りだけ)</h2>
      <p>
        3NF は「非キー属性同士の推移」を排除するが、
        <strong>候補キーが複数存在する場合の候補キー間の従属</strong> は許容してしまう。
        これを排除する更に厳密な正規形が <strong>ボイス・コッド正規形 (BCNF)</strong> だ。
      </p>
      <p>
        「候補キーが複数あり、かつ候補キー間で従属関係が起きうる」設計は現実の業務モデルではそこまで頻出しないため、
        実務では 3NF まで正規化すれば十分とされることが多い。
        BCNF の詳細と例は別トピックで扱う (将来追加予定)。
      </p>

      <h2>実務で 3NF まで正規化する意義</h2>
      <ul>
        <li>更新時異常のほとんど (挿入・更新・削除の全パターン) が構造的に排除される</li>
        <li>マスタ (顧客・商品・部署・カテゴリなど) が独立したテーブルとして自然に切り出される</li>
        <li>後段の設計 (外部キー制約・インデックス設計・トランザクション設計) の議論が単純になる</li>
      </ul>

      <p>
        ここまで正規化した上で、
        「明らかに参照 &gt;&gt; 更新かつ JOIN コストが問題になる」局所についてのみ、
